...
class DocumentResource(BaseModel):
    ...
    name: str = Field(description="Business document name", examples=["Contract Q3 2026"])
    original_filename: str = Field(description="Original uploaded filename", examples=["contract.pdf"])
    mime_type: str = Field(description="Document MIME type", examples=["application/pdf"])
    size_bytes: int = Field(description="Document size in bytes", examples=[245678])
    storage_path: str = Field(description="Internal storage path", examples=["storage/documents/abc123.pdf"])
